...
<div class="page-wrapper">
	<div class="content">
		...
		<div class="row">
			<div class="col-lg-12">
				<div class="card-box"
					style="border-bottom: 3px solid darkorange; text-align: end;">
					<a href="ledgers.htm?invoice=${invoice}">
						<button class="btn btn-primary"
							style="border-radius: 0px !important; background-color: cornflowerblue;">Back</button>
					</a>
					<!-- <button class="btn btn-primary" style="border-radius: 0px !important; background-color: #4a274f;">CSV</button> -->
					<button class="btn btn-primary"
						onclick="getPDF('${ledger.invoiceNo}')" id="downloadbtn"
						style="border-radius: 0px !important; background-color: #4a274f; display: inline-block;">PDF</button>
					<button class="btn btn-primary"
						style="border-radius: 0px !important; background-color: #4a274f;"
						onclick="PrintMe('printDiv')">
						...
						<div class="panel-body">
							<div class="row" style="width: 100%; margin-left: 0px;">
								<div class="col-md-6 m-b-20">
									<img
										src="${pageContext.request.contextPath}/static/images/CompanyLogo.jpg"
										class="m-b-20" alt=""
										style="width: 69px; border-radius: 50%; height: 69px; border: 5px solid #f79753;">
							...
							<div class="row" style="width: 100%; margin-left: 0px;">
								<div class="col-md-6 col-lg-9 m-b-20">
									...
									<ul class="list-unstyled">
										...
										<li>${trainee.addressDto.line2},
											${trainee.addressDto.pincode}</li>
								...
								<div class="col-md-6 col-lg-3 m-b-20">
									...
									<ul class="list-unstyled invoice-payment-details">
										<li><h5>
												Total Due: <span class="text-right">&#8377;
													${ledger.amount}</span>
											</h5></li>
										<li>A/C Name: <span><c:if test="${not empty crAc}">${ledger.crAc.code} - ${ledger.crAc.name}</c:if>
												<c:if test="${not empty drAc}">${ledger.drAc.code} - ${ledger.drAc.name}</c:if></span></li>
										<li>A/C No: <span><c:if test="${not empty crAc}">${ledger.crAc.code} - ${ledger.crAc.name}</c:if>
												<c:if test="${not empty drAc}">${ledger.drAc.code} - ${ledger.drAc.name}</c:if></span></li>
								...
								<table class="table table-striped table-hover">
									...
									<tbody>
										<tr>
											...
											<td><c:if test="${not empty crAc}">${ledger.crAc.code} - ${ledger.crAc.name}</c:if>
												<c:if test="${not empty drAc}">${ledger.drAc.code} - ${ledger.drAc.name}</c:if></td>
											<td><c:if test="${not empty crAc}">${ledger.crAc.code} - ${ledger.crAc.name}</c:if>
												<c:if test="${not empty drAc}">${ledger.drAc.code} - ${ledger.drAc.name}</c:if></td>
											<td>&#8377; ${ledger.amount}</td>
											<td>&#8377; ${ledger.amount}</td>
							...
							<div>
								...
								<div class="invoice-info">
									<h5>NOTICE:</h5>
									<p class="text-muted">A finance charge of 1.5% will be made
										on unpaid balances after 30 days.</p>
								</div>
							</div>
